...
export default function CalendarView({ events, initialYear, initialMonth }: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-y-2 mb-3 lg:mb-4">
        <div className="flex items-center gap-3">
          <button onClick={prevMonth} className="p-1.5 rounded hover:bg-gray-100 text-gray-600">‹</button>
          <h2 className="text-base lg:text-lg font-semibold text-gray-900 w-36 lg:w-44 text-center">
            {MONTHS[month]} {year}
          </h2>
      ...
      {view === 'month' ? (
        <div className="flex flex-col lg:flex-row gap-3 lg:gap-4 flex-1 min-h-0 overflow-y-auto lg:overflow-visible">
          {/* Grid. flex-1 only from lg, where the row layout needs it to take
          ...
          <div className="w-full shrink-0 lg:w-auto lg:flex-1 min-w-0">
            ...
            <div className="grid grid-cols-7 gap-px bg-gray-200 rounded-lg overflow-hidden flex-1">
              {cells.map((day, i) => {
                if (!day) return <div key={i} className="bg-gray-50 h-16 lg:h-24" />
                ...
                return (
                  <div
                    ...
                    className={`bg-white h-16 lg:h-24 p-1 cursor-pointer hover:bg-blue-50 transition-colors ${selected ? 'ring-2 ring-inset ring-blue-500' : ''}`}
                  >
                    <div className={`text-xs font-medium w-5 h-5 lg:w-6 lg:h-6 flex items-center justify-center rounded-full mb-0.5
                    ...
                    <div className="hidden lg:block space-y-0.5 overflow-hidden">
          ...
          {/* Day detail panel */}
          <div className="w-full lg:w-72 lg:shrink-0 border border-gray-200 rounded-lg p-3 lg:overflow-y-auto">
            {selectedDay ? (
              <>
